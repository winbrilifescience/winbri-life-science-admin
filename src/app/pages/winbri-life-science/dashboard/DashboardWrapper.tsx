...
import { GetGeneralDashboard } from '../../../Functions/WinbriLifeScience'
...
const DashboardWrapper = () => {
	...
	const fetchOrderData = async () => {
		...
		try {
			...
			// 	item_type: ['BOOKS', '_PRODUCT', 'FITNESS_COURSE', 'EBOOKS'],
			// 	order_status: ['SUCCESS'],
			// 	currency: 'INR',
			// 	purchase_mode: ['ONLINE', 'Cash On Delivery'],
			// 	gateway: ['RAZORPAY', 'RAZORPAY_WINBRI'],
			// 	from_date: startDate,
			// 	to_date: endDate,
			// })
			let response: any
			...
			const winbriMealProduct = response?.data?.filter(
				(item: any) => item.item_type === ''
			)
			const winbriMealProductTotal = winbriMealProduct?.reduce(
				(acc: any, item: any) => acc + item.total_amount,
				0
			)
			const winbriMealProductCount = winbriMealProduct?.reduce(
			...
			setTotalData({
				...
				winbriMealProduct: {
					total: winbriMealProductTotal,
					count: winbriMealProductCount,
				},
				fitnessCourse: {
					...
					<div className='col-md-12'>
						<div className='card p-5'>
							...
							<div className='row'>
								...
								<div className='col-xl-4'>
									<TotalProductOrder
										dashboardData={totalData?.winbriMealProduct?.count}
										className='mb-xl-8'
										color='success'
										Amount={totalData?.winbriMealProduct?.total ? totalData?.winbriMealProduct?.total : 0}
									/>
								</div>
								<div className='col-xl-4'>
									<TotalFitnessOrder
